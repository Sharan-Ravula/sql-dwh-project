...
```text
`You Write the Code`(On your Mac) -> `VSCode Sends the Command`(Over the Internet)
                     ^                                        |
                     |                                        v
`VSCode Displays the Result`(Back on your Mac) <- `Azure SQL Executes the Command`(In the Cloud)
```
